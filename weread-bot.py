...
NOTIFICATION_CHANNEL_REQUIRED_FIELDS = {
    ...
    "gotify": ["server", "token"],
    "serverchan3": ["sendkey"],  # uid 仅 Server酱³(sctp 开头)必需，Turbo(SCT 开头)无需
    "pushdeer": ["pushkey"],
}
...
class NotificationService:
    ...
    def _send_serverchan3(self, message: str, config: Dict[str, Any]) -> bool:
        """发送Server酱通知（兼容 Turbo 与 SC3 两种 SendKey）

        - Server酱Turbo: SendKey 以 SCT 开头, 推送微信, 接口 sctapi.ftqq.com
        - Server酱³: SendKey 以 sctp 开头, 推送独立App, 接口 {uid}.push.ft07.com
        """
        ...
        if sendkey.lower().startswith("sctp"):
            # Server酱³ (SC3): 需要 uid
            uid = (config.get("uid") or "").strip()
            if not uid:
                logging.error("❌ Server酱³ UID未配置（sctp 开头 SendKey 需填 SERVERCHAN3_UID）")
                return False
            url = f"https://{uid}.push.ft07.com/send/{sendkey}.send"
            service_name = "Server酱³"
        else:
            # Server酱Turbo: 推送微信
            url = f"https://sctapi.ftqq.com/{sendkey}.send"
            service_name = "Server酱Turbo"

        # Server酱官方接口使用 form 编码 (application/x-www-form-urlencoded)
        data = {
            "title": "WeRead Bot 通知",
            "desp": message,
        }
        ...
        try:
            ...
            if result.get("code") == 0:
                logging.info(f"✅ {service_name}通知发送成功")
                return True
            logging.error(f"❌ {service_name}通知发送失败: {result}")
            return False
        except Exception as e:
            logging.error(f"❌ {service_name}通知发送失败: {type(e).__name__}: {e}")
            return False
